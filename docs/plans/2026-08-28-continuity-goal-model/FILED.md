...
## F10h — the opening square bracket enters `MARKDOWN_LEADING_CHARS` through the same door the equals character used, so its idempotency is unwitnessed for the reason `F10g` records

- **Surfaced by:** U10 criterion 5 implementation (the link reference definition forgery).
- **Evidence:** criterion 5 adds `'['` to `MARKDOWN_LEADING_CHARS` at `src/render/escape.ts:7`, which now reads ``new Set(['#', '-', '*', '+', '>', '`', '~', '_', '=', '['])``. `F10g` is written against the equals character alone — its heading, its evidence and its remedy each name `'='` and nothing wider — so it does not already cover this character, and this entry records the second drift rather than restating the first. The mechanism is identical. The set is one of the four disjuncts of `isEmittedEscape` (`src/render/escape.ts:23-27`), so the bracket enters the emitted-escape population as well as the line-start branch of `escapeStored` (`:66-72`). The two censuses that derive their populations live from `isEmittedEscape` through `collectEmittedPopulation` (`test/unit/escape.test.ts:218-225`) — `escape.round-trip-census-over-the-emitted-escape-set` (`:232`) and `escape.emitted-token-alphabet-is-prefix-free` (`:243`) — absorb the bracket with no edit. The idempotency census does not. `escape.stored-is-idempotent-over-the-escapable-and-markdown-leading-population` (`:207`) draws its population from `collectIdempotencyPopulation` (`:201-205`), which reads the file's own local `MARKDOWN_LEADING_CHARS` array at `test/unit/escape.test.ts:199` — the hand-copy `F10d` files as drift and `F10g` files as having drifted once already — and that array still holds the eight characters the module held before criterion 4. The bracket is therefore absent from the idempotency population, exactly as the equals character is, and no test observes either one there. The property itself holds: measured on this branch, `escapeStored('[')` returns `'U+005B'` and `escapeStored('U+005B')` returns `'U+005B'` unchanged, so escaping twice equals escaping once. What is missing is the witness, not the behaviour. The escape's scope is unchanged elsewhere — `'['` is not added to `ANGLE_BRACKETS` and not routed through `isEscapable`, so it is neutralised only at a line start and ordinary prose is untouched; measured, `escapeStored('see [the plan] for detail')` returns `'see [the plan] for detail'`.
- **Why it is above the ceiling:** the repair is the one `F10g` already names — deleting the local array at `test/unit/escape.test.ts:199` in favour of the module's set, or editing that array's contents — and both are changes to a shipped test this criterion does not own, which the U10 work order forbids. `F10d` holds the repair and names the exports (`isEmittedEscape`, `toEscaped`) that make it possible. Recorded here so the hand-copy's drift is counted at two characters rather than one on the record `F10d` will be closed against.
- **Not folded in.**

## F10i — the escape-token ambiguity criterion 7 names is already closed across the whole emitted population, and the residue is that the suite witnesses it only by hand-picked example

- **Surfaced by:** U10 criterion 7 investigation.
- **Evidence:** criterion 7 asks that escape-token ambiguity when a digit follows be "resolved, or accepted with a recorded rationale", and it is resolved, so the criterion is discharged as written and owes no code. `toEscaped` pads to a minimum of four hexadecimal digits and never truncates, so any code point below `U+1000` yields a four-digit token beginning with `0`, which can neither be a prefix of a longer token nor have a longer token as its prefix. An exhaustive scan of all 1,112,064 non-surrogate code points found 764 whose addition to the escaped set would break prefix-freedom; the lowest is `U+110B`. Neither `=` (`U+003D`) nor `[` (`U+005B`) is among them, so this unit's two widenings — the ones `F10g` and `F10h` record — cannot reopen it. Confirmed by execution: with both characters added the emitted set is 267, and a 267 x 16 adjacency census over every emitted character followed by every uppercase hexadecimal digit reports zero broken pairs; `U+003DA` decodes to `=A` and `U+005BB` decodes to `[B`. The residue, which is the reason to file rather than simply close: the shipped adjacency test is hand-picked, five assertions over three characters out of 265, and the round-trip census does not cover for it, because none of its five contexts places a hexadecimal digit after a token. A stacked mutation that reverses `ESCAPE_WIDTHS` (`src/render/escape.ts:94`, which reads `const ESCAPE_WIDTHS = [4, 5, 6] as const`) to `[6, 5, 4]` and deletes the canonical-spelling guard at `src/render/escape.ts:109`, which reads ``if (toEscaped(char) !== `${ESCAPE_PREFIX}${digits}`) continue``, breaks 88 (emitted character, hexadecimal digit) pairs while all 17 shipped tests stay green. Neither guard alone is necessary: reversing the widths alone changes nothing observable, and deleting the canonical guard alone changes nothing observable. The closing work would be one live-derived census asserting `unescapeStored(escapeStored(char + d)) === char + d` over every emitted character and every hexadecimal digit, verified red-capable. The 764-code-point danger list was measured on Node v26.4.0 against Unicode 17.0 and is version-specific; the structural argument — a four-digit token beginning with `0` cannot collide either way — is not.
- **Why it is above the ceiling:** criterion 7 offers "resolved, or accepted with a recorded rationale", and it is resolved, so this record is the criterion's deliverable rather than a deferral of it. Proving the mechanism inside the suite is a strictly stronger property no criterion asks for, and the census that would prove it is a new test rather than the repair of a failing one.
- **Not folded in.**

## F10j — the recorded sentinel-invariance risk is mis-specified in both its trigger and its symptom, and the invariant the fixture depends on is asserted nowhere

- **Surfaced by:** U10 criterion 7 investigation.
- **Evidence:** the thread recorded the risk as "nothing asserts that the refusal fixture's sentinel key stays invariant under the escape. The next widening that covers its first character will fail eleven tests with a message that names neither the sentinel nor the cause." Two corrections, both measured. First, the count of eleven is exactly right and the tests are the per-tool `.rejects-invalid` tests, but the message does name the sentinel — it interpolates the field on both sides, reading `expected the refusal to name field "#logbook-unexpected-field", got "field: U+0023logbook-unexpected-field"`. What it hides is the cause: it reads as a server-side regression in the refusal renderer rather than as one character added to a set in a different module. Second, and the more useful correction, "covers its first character" is the wrong trigger. The real condition is `escapeStored(key) !== key`. The key is `logbook-unexpected-field` at `test/support/schema-arbitrary.ts:202`, which reads `const unknownKey = 'logbook-unexpected-field'`, and its first character is `l` (`U+006C`); because it carries two hyphens mid-string, a widening that escapes an existing markdown character at every position — as commit `9555b8e4` already did for angle brackets — fires all eleven without touching the first character. Demonstrated: an every-position hyphen widening yields `logbookU+002DunexpectedU+002Dfield`. The key reaches `escapeStored` at exactly one place, `src/schema/refusal.ts:37`, and five test files then compare the escaped key against the raw key held in the fixture; that comparison passes only because the key happens to be invariant under the escape, which nothing states and nothing checks. Measured for this unit: full suite runs with `=` added and with `[` added were each identical to baseline, so neither of this unit's two widenings fired it. The repair is one assertion pinning `escapeStored('logbook-unexpected-field') === 'logbook-unexpected-field'`, or the fixture reading its expected field through `escapeStored` rather than holding it raw.
- **Why it is above the ceiling:** it is a test-engineering repair to a fixture no criterion in this unit owns, and what it corrects is a recorded risk rather than a criterion. This unit's two widenings were measured not to fire it, so nothing is failing now and nothing here is a regression this unit introduced.
- **Not folded in.**

## F10k — criterion 6 cannot be honestly implemented as written, because every reading of it either breaks a shipped test or changes nothing observable

- **Surfaced by:** U10 criterion 6 investigation.
- **Evidence:** criterion 6 asks that the indent detector cover tabs, not only spaces. Four readings were measured over a 7382-input population plus the two real fixture payloads. Making the line-start state survive a leading tab changes 310 outputs and breaks the shipped `render.spine-fields-cannot-forge-structure`, whose fixture at `test/spawn/forgery.test.ts:511` pins `{ stored: '\t- forged bullet from a tab', neutralised: 'U+0009- forged bullet from a tab' }` — the change would render `U+0009U+002D forged bullet from a tab`. Generalising the blank run to a horizontal-whitespace set produces byte-identical output to that reading and breaks the same test. Making a tab advance the indent count changes zero of 7382 outputs, and that zero is a control-flow proof rather than a sample: `spaceRun` is read in exactly one place, guarded by the line-start flag, and the general path clears that flag for a tab and resets the counter on the next character, so a count a tab contributes can never be read before it is overwritten. A fourth reading, using a separate indent flag, changes 135 outputs without breaking a test, but every difference has the shape "a space following an escaped tab is now escaped too", converting an already-inert line into a differently-inert line — once a tab becomes the token `U+0009` the line begins with the letter `U`, so nothing after it is indentation at all. A whitespace census settles it: all 25 code points JavaScript's `\s` matches are already inside the union `isEscapable` covers, and run through the escape at a line start the only whitespace character surviving unescaped is the ordinary space, which the counter already handles. Finally, the future change criterion 6 guards against is already caught: mutating `isBlank` so that tabs stop being escaped turns `escape.covers-both-classes` red, at 21 tests, 20 pass, 1 fail.
- **Why it is above the ceiling:** the two coherent resolutions are to strike criterion 6, or to restate it as already satisfied with `escape.covers-both-classes` as its receipt and that tab-only mutation as its inertness proof. Both are scope rulings the orchestrator owns; neither is a move an implementer may make against a criterion as written, and the third option — shipping one of the four readings — is either a broken shipped test or a change with no observable effect. Recorded here pending that ruling.
- **Not folded in.**

## F10l — the briefing renderer's missing blank line is load-bearing for the forgery criterion 4 closed, and the layout half of that defence is asserted nowhere

- **Surfaced by:** U10 criterion 4 architecture review.
- **Evidence:** `src/render/briefing.ts:357-358` places a stored value directly beneath a server-authored sentence with no blank line between them: line 357 emits `LEGACY_LAST_SESSION_MARKER` and line 358 emits `escapeStored(value)`, carrying none of the `.map(() => '')` separators that every neighbouring block in the same array carries. Four shipped tests pin that adjacency. The adjacency is what made the setext-heading forgery reachable rather than theoretical, and criterion 4 closed it by adding the equals character to the escaped set — the same widening `F10g` records. The residue is that the layout half stays unasserted: if someone later removes the equals character from the escaped set, nothing at that call site will catch it.
- **Why it is above the ceiling:** criterion 4's criterion is discharged by neutralising the character, and pinning renderer layout is a second, independent mechanism against the same threat rather than a repair to the first. Asserting it means pinning a property of the briefing renderer that no U10 criterion states.
- **Not folded in.**

## F10m — a second call site relies on an unasserted blank line, and unlike the first its exploitability was never measured

- **Surfaced by:** U10 criterion 4 architecture review.
- **Evidence:** `src/server/resource-render.ts:47` carries a blank line whose presence is relied upon and not asserted — it is the `''` that `renderSessionEntryResource` places between its four server-authored header lines and `escapeStored(entry.body)` on the line below. That is the same shape as `F10l`, a renderer's line layout relied upon and not asserted, at a different call site. Recorded honestly: this one was identified by inspection during the criterion 4 architecture review, and its exploitability was not measured, unlike `F10l`'s. Whether removing that blank line would make a forgery reachable here is therefore unknown, and this entry does not claim it would.
- **Why it is above the ceiling:** the same reason as `F10l` — criterion 4 is discharged by neutralising the character, and pinning renderer layout is a second mechanism against the same threat, which no U10 criterion states.
- **Not folded in.**
