...
## Amendments, made after MSP-2 shipped

This section is written after MSP-2 merged at `b8506c67`. It corrects this plan without rewriting the text it corrects — the sentences at `:791`, `:819` and `:1059` stay exactly as written, because the negative answers below are only meaningful next to the questions that asked for them. Where this plan and the code disagree, the code wins.

1. **The payload claim at `:791` is false, and the plan's own guard caught it.** `:791` records the `SubagentStop` shape as unverified and sourced from external documentation, naming `agent_id` and `agent_type` as present and denying a `stop_hook_active` equivalent. `:819` required a capture before any code proceeded and instructed: stop and record a decision if the capture disagreed on any of the three fields. The capture disagreed on the third — `stop_hook_active` is present, carrying `false` (`test/fixtures/hook-events/subagent-stop.json:12`) — while `agent_id` and `agent_type` are both present, as the plan asserted. That instruction was honoured: decision `01M1YYTGQNNMCKJFB4FSSQVPDJ` records the disagreement. Corrected in the spec errata as `E1`.

2. **The ambiguity named at `:1059` was confirmed wrong, not right.** The plan asked for confirmation that the first subagent of a session uses the same reference head as the `Stop` gate, and said only that this file would change if the reading were wrong. The confirmation came back negative — decision `01M1YZA2377NZEHEMR0EZC4BAV` — and the reference head was removed from the subagent gate entirely rather than re-pointed: `6724fbc4` dropped the gate's own head write as a no-op, and `10d5c266` then dropped the head comparison itself. The gate is damped instead by a per-`agent_id` marker file, so every subagent is asked rather than only the first. Corrected in the spec errata as `E3` and `E4`.

3. **Two of the three open questions in "What this plan does not settle" are now settled.** "Whether the reference head for the first subagent gate is right" is settled by item 2 above: it was wrong, and there is now no reference head left to be right or wrong about. "Whether the captured `SubagentStop` payload matches the spec" is settled by item 1: it does not, on `stop_hook_active`. The third question — what wins on a merge when two clones disagree about settledness — is untouched and remains open.

4. **A finding the plan did not contemplate.** A `SubagentStop` event does not always mean a subagent finished. An event carrying an empty `agent_type` has no matching subagent start; it marks an internal fork of the main thread, and the merged gate refuses those events (`src/hooklib/subagent-stop-gate.ts:56`). Decision `01M1YYTGQNNMCKJFB4FSSQVPDJ` records it. Corrected in the spec errata as `E2`.
