...
allowed-tools: mcp__ledger__append_session_event, mcp__ledger__record_decision, mcp__ledger__update_thread, mcp__ledger__transition_thread, mcp__ledger__rebuild_index
---

# Session Handoff

Write the ledger through the `ledger` MCP tools. The ledger is canonical; the chat summary is a courtesy for a future session that starts with zero context. This skill only orchestrates the tools — the server owns every rule (lifecycle, gates, caps, validation) and refuses anything illegal. The read side is the `resume-project` skill.
...
6. Rebuild the index. Call `mcp__ledger__rebuild_index` so the derived roster reflects this hand-off.

7. Print the hand-off summary. In chat, state the thread worked, its new status, the next step you recorded, and any decisions added. The ledger is the source of truth; this summary is the courtesy copy.
